...
def create_agent(pdfs_folder: str, llm_model: str = "claude-sonnet-4-20250514",
    ...
    agent = ReActAgent(
        name="PDF_Analyzer",
        description="An agent that can answer questions about insurance PDF documents",
        tools=tools,
        llm=llm,
        verbose=verbose
    )
...
"""
    ...
    # Query the agent using run() with proper async handling
    import asyncio

    async def async_run():
        result = await agent.run(user_msg=enhanced_prompt)
        return result
    ...
    response = loop.run_until_complete(async_run())
